package dev.yamh.io.presentation.core.ui.source.kit.atom.indicator
...
public enum class PagerIndicatorOrientation {
    Horizontal,
    Vertical,
}
...
internal fun PagerIndicatorInternal(
    ...
    minDotSize: Dp = 4.dp,
    space: Dp = 8.dp,
    orientation: PagerIndicatorOrientation = PagerIndicatorOrientation.Horizontal,
    ...
    Canvas(
        modifier = modifier
            .width(if (orientation == PagerIndicatorOrientation.Horizontal) mainAxisSize else activeDotSize)
            .height(if (orientation == PagerIndicatorOrientation.Horizontal) activeDotSize else mainAxisSize)
    ) {
        ...
        translate(
            left = if (orientation == PagerIndicatorOrientation.Horizontal) scroll else 0f,
            top = if (orientation == PagerIndicatorOrientation.Vertical) scroll else 0f,
        ) {
            ...
            for (i in firstVisible..lastVisible) {
                ...
                val left =
                    if (orientation == PagerIndicatorOrientation.Horizontal) mainAxisStart else crossAxisStart
                val top =
                    if (orientation == PagerIndicatorOrientation.Horizontal) crossAxisStart else mainAxisStart
    ...
    return this.pointerInput(Unit) {
        detectTapGestures { offset ->
            ...
            val fraction = when (orientation) {
                PagerIndicatorOrientation.Horizontal -> offset.x / size.width
                PagerIndicatorOrientation.Vertical -> offset.y / size.height
            }
