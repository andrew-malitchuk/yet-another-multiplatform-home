 ...
 */
@Composable
public fun VerticalMultiBrowseCarousel(
    state: CarouselState,
    ...
    content: @Composable CarouselItemScope.(itemIndex: Int, isCollapsed: Boolean) -> Unit
) {
    BoxWithConstraints {
            ...
            keylineList = { availableSpace, itemSpacingPx ->
                with(density) {
                    multiBrowseKeylineList(
                        ...
                        preferredItemSize = preferredItemHeight.toPx(),
                        itemCount = state.itemCountState.value.invoke(),
                        itemSpacing = itemSpacingPx,
                        minSmallItemSize = CarouselDefaults.MinSmallItemSize.toPx(),
                        maxSmallItemSize = this@BoxWithConstraints.maxWidth.toPx()
                    )
                }
            },
...
internal fun Carousel(
    ...
    val beforeContentPadding = contentPadding.calculateBeforeContentPadding(Orientation.Vertical)
    val afterContentPadding = contentPadding.calculateAfterContentPadding(Orientation.Vertical)
    val pageSize =
    ...
    VerticalPager(
        ...
        Box(
            ...
            Text("isCollapsed=${carouselItemInfo.isCollapsed}")
            scope.content(page, carouselItemInfo.isCollapsed)
        }
    }
}
...
//                        size.height / 2f
//                    val centerY =
//                        strategyResult.itemMainAxisSize / 2f
//                    val halfMaskWidth =
//                        size.width / 2f
//                    val halfMaskHeight =
//                        interpolatedKeyline.size / 2f
//                    val maskRect =
//                        Rect(
